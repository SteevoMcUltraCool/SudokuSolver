    ...
    operate: function (FilledBoxes, UnfilledBoxes, Grids) {
      ...
      Object.values(Grids)
        ...
        .forEach((grid) => {
          ...
          let localFilledBoxes = grid.Boxes.filter(
            (box) => box && box.getValue()
          );
          let localunfilledBoxes = grid.Boxes.filter(
            (box) => box && !box.getValue()
          );
          localFilledBoxes.forEach(
            (box) => (valuesRemaining[box.getValue()] = null)
          );
          valuesRemaining = valuesRemaining.filter((value) => value);
          valuesRemaining.forEach(function (v) {
            let boxesWith = localunfilledBoxes.filter((box) =>
              box.potetnialValues.includes(v)
            );
            if (boxesWith.length == 2) {
              valuesRemaining
                .filter((val) => val != v)
                .forEach(function (val) {
                  boxesWith = boxesWith.filter((box) =>
                    box.potetnialValues.includes(val)
                  );
                  let boxWithReduced = localunfilledBoxes.filter((box) =>
                    box.potetnialValues.includes(val)
                  );
                  if (boxesWith.length == 2 && boxWithReduced.length == 2) {
                    //elimination time
                    if (
                      PerformedOperations.includes(
                        "DEIG" +
                          boxesWith[0].row +
                          boxesWith[0].column +
                          boxesWith[1].row +
                    ...
                    if (boxesWith[0].row == boxesWith[1].row) {
                      ...
                      Boxes[row]
                        .filter((box) => box && !box.getValue())
                        .forEach((box) => {
                          box.potetnialValues[v] = null;
                          box.potetnialValues[val] = null;
                        });
                    } else if (boxesWith[0].column == boxesWith[1].column) {
                    ...
                    console.log(
                      boxesWith[0].potetnialValues.toString(),
                      boxesWith[1].potetnialValues.toString()
                    );
                    //elimination time
                    new Move(
                      "Double Elimination in Grid: ",
                      [boxesWith[0], boxesWith[1]],
                      DOMmoves
                    );
                  }
                });
