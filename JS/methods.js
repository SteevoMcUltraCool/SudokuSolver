...
const methods = {
    LastNumberRemaining: {
        ...
        getDescription: n => `Pay attention to the highlighted cells. Since all numbers except ${n} can be eliminated, we know this box must be ${n}`,
        difficulty: 1,
    ...
    LastCellRemainingInGrid: {
        ...
        getDescription: n => `Pay attention to the highlighted rows and columns. Since no other box in the grid can be ${n}, this box must be ${n}`,
        difficulty: 2,
        difficultyDepracatives:["LastCellRemainingInRow","LastCellRemainingInColumn"],
        operate: function(FilledBoxes, UnfilledBoxes,Grids){
            let change = false
    ...
    LastCellRemainingInRow: {
        ...
        getDescription: n => `Pay attention to the highlighted columns and grids. Since no other box in the row can be ${n}, this box must be ${n}`,
        difficulty: 2,
        difficultyDepracatives:["LastCellRemainingInGrid","LastCellRemainingInColumn"],
        operate: function(FilledBoxes, UnfilledBoxes,Grids){
            let change = false
    ...
    LastCellRemainingInColumn: {
        ...
        getDescription: n => `Pay attention to the highlighted rows and grids. Since no other box in the column can be ${n}, this box must be ${n}`,
        difficulty: 2,
        difficultyDepracatives:["LastCellRemainingInGrid","LastCellRemainingInRow"],
        operate: function(FilledBoxes, UnfilledBoxes,Grids){
            let change = false
            let Boxes = Box.prototype.getBoxes()
            let Columns = Grid.prototype.rotate(Boxes)
    ...
    SimpleMultiEliminationFromGrid: {
        ...
        getDescription: (n1,n2,n3) => `Since the numbers remaining in this row of this grid must be ${n1} & ${n2}${(n3 && " & "+n3 )|| ""}, we can eliminate these valuse from the rest of the row.`,
        difficulty: 1.4,
        difficultyDepracatives:["SimpleMultiEliminationFromRow","SimpleMultiEliminationFromColumn"],
        operate: function(FilledBoxes, UnfilledBoxes,Grids){
            let change = false
            Object.values(Grids).forEach(grid => {
                ...
                if (poppy.length <= 3){
                    ...
                    }else if ((poppy[0].column == poppy[1].column ) && (poppy[1].column==poppy[2].column)) {
